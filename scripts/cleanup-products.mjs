// 清理 products 表重复数据并添加唯一约束
import { createClient } from '@supabase/supabase-js';

const supabaseUrl = 'https://isefskqnkeesepcczbyo.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImlzZWZza3Fua2Vlc2VwY2N6YnlvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMyMjYzMzEsImV4cCI6MjA4ODgwMjMzMX0.MNRD-6mK1ML3fBqupvX3Cet1lL4N9Hc9zCAB_4EbWNE';

const supabase = createClient(supabaseUrl, supabaseKey);

async function cleanupProducts() {
  console.log('🔍 查询所有商品...\n');
  
  const { data: products, error } = await supabase
    .from('products')
    .select('*')
    .order('created_at', { ascending: true });
  
  if (error) {
    console.error('❌ 查询失败:', error.message);
    return;
  }
  
  console.log(`📦 总商品数：${products.length}`);
  
  // 按名称 + 分类分组
  const keyMap = new Map();
  products.forEach(product => {
    const key = `${product.name.trim()}__${product.category || 'default'}`;
    if (!keyMap.has(key)) {
      keyMap.set(key, []);
    }
    keyMap.get(key).push(product);
  });
  
  // 找出重复的
  let duplicateCount = 0;
  const toDelete = [];
  
  keyMap.forEach((items, key) => {
    if (items.length > 1) {
      duplicateCount++;
      const [name, category] = key.split('__');
      console.log(`\n⚠️  重复：${name} (${category}) - ${items.length} 个`);
      
      // 保留最早创建的
      const keep = items[0];
      const duplicates = items.slice(1);
      
      console.log(`   ✅ 保留：${keep.id}`);
      duplicates.forEach(item => {
        console.log(`   ❌ 删除：${item.id}`);
        toDelete.push(item.id);
      });
    }
  });
  
  if (duplicateCount === 0) {
    console.log('\n✅ 没有发现重复商品！');
    return;
  }
  
  console.log(`\n📊 统计：${duplicateCount} 组重复，共 ${toDelete.length} 条待删除\n`);
  console.log('⏸️  等待 3 秒后开始删除... (按 Ctrl+C 取消)\n');
  await new Promise(resolve => setTimeout(resolve, 3000));
  
  // 批量删除（每批 50 个）
  console.log('🗑️  开始删除重复商品...');
  let successCount = 0;
  let failCount = 0;
  const batchSize = 50;
  
  for (let i = 0; i < toDelete.length; i += batchSize) {
    const batch = toDelete.slice(i, i + batchSize);
    const promises = batch.map(id => 
      supabase.from('products').delete().eq('id', id)
    );
    
    const results = await Promise.all(promises);
    results.forEach(({ error }) => {
      if (error) failCount++;
      else successCount++;
    });
    
    const progress = Math.min(i + batchSize, toDelete.length);
    console.log(`   进度：${progress}/${toDelete.length} (成功：${successCount}, 失败：${failCount})`);
  }
  
  console.log(`\n✅ 完成！成功删除 ${successCount} 个，失败 ${failCount} 个`);
  console.log(`📦 剩余商品数：${products.length - successCount}`);
  
  // 验证结果
  console.log('\n🔍 验证结果...');
  const { data: remaining } = await supabase.from('products').select('*');
  const remainingMap = new Map();
  remaining.forEach(p => {
    const key = `${p.name.trim()}__${p.category || 'default'}`;
    if (!remainingMap.has(key)) remainingMap.set(key, 0);
    remainingMap.set(key, remainingMap.get(key) + 1);
  });
  
  let stillDuplicate = 0;
  remainingMap.forEach((count, key) => {
    if (count > 1) stillDuplicate++;
  });
  
  if (stillDuplicate === 0) {
    console.log('✅ 验证通过：无重复商品！');
  } else {
    console.log(`⚠️  仍有 ${stillDuplicate} 组重复，可能需要手动处理`);
  }
}

cleanupProducts();
